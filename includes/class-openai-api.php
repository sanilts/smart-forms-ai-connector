...
/**
 * OpenAI API Class - UPDATED with Dynamic Chunking Settings
 * 
 * Handles API requests to the OpenAI API and tracks token usage
 */
...
class SFAIC_OpenAI_API {
    ...
    public function make_request($messages, $model = null, $max_tokens = 1000, $temperature = 0.7) {
        ...
        if (is_wp_error($response)) {
            error_log('CGPTFC: OpenAI API Request Error: ' . $response->get_error_message());
            return $response;
        }
        ...
        if ($response_code !== 200) {
            ...
            if ($response_code === 401) {
                return new WP_Error('api_error', __('Invalid API key. Please check your OpenAI API key in settings.', 'chatgpt-fluent-connector'));
        ...
        if (!isset($response_data['choices'][0]['message']['content'])) {
            error_log('CGPTFC: Invalid OpenAI response structure: ' . wp_json_encode($response_data));
            return new WP_Error('invalid_response', __('Invalid response structure from OpenAI API', 'chatgpt-fluent-connector'));
        }
    ...
    public function get_response_content($response) {
        ...
        if (!isset($response['choices'][0]['message']['content'])) {
            return new WP_Error('invalid_response', __('Invalid response from OpenAI API', 'chatgpt-fluent-connector'));
        }
    ...
    public function process_form_with_prompt($prompt_id, $form_data, $entry_id = null) {
        ...
        if ($needs_chunking) {
            error_log('SFAIC OpenAI: Using chunked processing - estimated length: ' . $estimated_prompt_length);
        ...
        // For non-chunked responses, cap based on model
        $model_limits = array(
            'gpt-3.5-turbo' => 4096,
            'gpt-4' => 8192,
            'gpt-4-turbo' => 4096,
        ...
        if (is_wp_error($response)) {
            error_log('CGPTFC: Error in OpenAI API response: ' . $response->get_error_message());
            return $response;
        }
    ...
    public function process_form_with_prompt_chunked($prompt_id, $form_data, $entry_id = null) {
        error_log('SFAIC: Starting OpenAI chunked processing with dynamic settings');

        // Get chunking settings from prompt
        $chunking_settings = $this->get_chunking_settings($prompt_id);
        
        error_log('SFAIC: OpenAI Chunking settings: ' . json_encode($chunking_settings));
        ...
        $max_chunks = min(ceil($target_tokens / $chunk_size), 40);
        ...
        error_log("SFAIC: OpenAI dynamic chunking - target: {$target_tokens}, chunk: {$chunk_size}, max chunks: {$max_chunks}");
        ...
        for ($chunk_num = 0; $chunk_num < $max_chunks; $chunk_num++) {
            ...
            error_log("SFAIC: OpenAI Chunk " . ($chunk_num + 1) . " requesting {$current_chunk_size} tokens");
            ...
            error_log("SFAIC: OpenAI Chunk " . ($chunk_num + 1) . " used {$chunk_tokens_used} tokens. Total: {$total_tokens_used}");
        ...
        if (!empty($entry_id)) {
            ...
            update_post_meta($entry_id, '_openai_completion_reason', 'dynamic_chunking');
            update_post_meta($entry_id, '_openai_chunking_settings_used', $chunking_settings);
        }

        error_log("SFAIC: OpenAI dynamic chunking complete. " . ($chunk_num + 1) . " chunks, {$total_tokens_used} tokens, " . strlen($full_response) . " chars");
    ...
    /**
     * Format all form data into a structured text for OpenAI
     *
