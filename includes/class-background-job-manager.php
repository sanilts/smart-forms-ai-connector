<?php
/**
 * FIXED Background Job Manager Class - Resolves Stuck Jobs and Refresh Issues
 * 
 * Handles background processing of AI requests using WP Cron with improved reliability
 */
...
class SFAIC_Background_Job_Manager {
    ...
    private function init_hooks() {
        ...
        // FIXED: Add periodic stuck job cleanup
    ...
    /**
     * FIXED: Enhanced job scheduling with better reliability
     */
    public function schedule_job($job_type, $prompt_id, $form_id, $entry_id, $job_data, $delay = 0, $priority = 0) {
        ...
        // FIXED: Improved cron event scheduling
        $this->schedule_cron_event_improved($delay);
    ...
    /**
     * FIXED: Improved cron event scheduling with better reliability
     */
    private function schedule_cron_event_improved($delay = 0) {
        $timestamp = time() + $delay;

        // FIXED: Less restrictive scheduling - only check for very close events
        $next_scheduled = wp_next_scheduled(self::CRON_HOOK);

        if ($next_scheduled && $next_scheduled <= time() + 5) {
            error_log('SFAIC: Cron already scheduled very soon at ' . date('Y-m-d H:i:s', $next_scheduled) . ', skipping');
            return;
        }

        // FIXED: Don't clear existing events unless they're too far in the future
        if ($next_scheduled && $next_scheduled > time() + 300) { // Only clear if more than 5 minutes away
            wp_clear_scheduled_hook(self::CRON_HOOK);
            error_log('SFAIC: Cleared distant cron event scheduled for ' . date('Y-m-d H:i:s', $next_scheduled));
        }
    ...
    public function process_background_job() {
        ...
        if (!$job) {
            ...
            // FIXED: Better handling of future jobs
        ...
        // FIXED: Always schedule next job processing
        $this->schedule_next_job_processing();
    }

    /**
     * FIXED: Improved stuck job detection and reset
     */
    private function reset_stuck_processing_jobs() {
        ...
        $stuck_jobs = $wpdb->query($wpdb->prepare(
            ...
            self::STATUS_PENDING, // Reset to pending, not failed
            'Job was stuck in processing - reset to pending',
            current_time('mysql'),
    ...
    /**
     * FIXED: Periodic cleanup of stuck jobs
     */
    public function cleanup_stuck_jobs_periodic() {
        $this->reset_stuck_processing_jobs();
        
        // Also check for jobs scheduled in the past but still pending
        global $wpdb;
        
        $old_pending = $wpdb->query($wpdb->prepare(
            "UPDATE {$this->jobs_table} 
             SET error_message = CONCAT(COALESCE(error_message, ''), ' [Reset: was pending for too long]'), 
                 updated_at = %s
             WHERE status = %s 
        ...
        if ($wpdb->rows_affected > 0) {
            error_log('SFAIC: Updated ' . $wpdb->rows_affected . ' old pending jobs');
            // Try to reschedule processing
            $this->schedule_cron_event_improved(5);
        }
    }

    /**
     * FIXED: Enhanced next job scheduling
     */
    private function schedule_next_job_processing() {
        ...
        if ($pending_count > 0) {
            ...
            $this->schedule_cron_event_improved(5); // Process next job in 5 seconds
        } else {
            // Check for future jobs
            $future_job = $wpdb->get_row($wpdb->prepare(
            ...
            if ($future_job) {
                $delay = strtotime($future_job->scheduled_at) - current_time('timestamp');
    ...
    /**
     * FIXED: Check if we should process jobs (prevent race conditions)
     */
    private function should_process_jobs() {
        ...
        // FIXED: Simplified concurrent job check
        global $wpdb;
        $max_concurrent = 2; // Reduced from 3 to 2 for better stability
    ...
    private function update_job_status($job_id, $status, $error_message = '') {
        ...
        if ($result === false) {
            error_log('SFAIC: Failed to update job status for job ID: ' . $job_id . ', Error: ' . $wpdb->last_error);
        }
    }

    // [Include all the existing helper methods like extract_user_name, extract_user_email, etc.]
    ...
    private function try_immediate_processing() {
        ...
        // Alternative: direct call if no cron scheduled
    ...
    /**
     * Process job immediately (fallback)
     */
    ...
    // [Include all remaining AJAX handlers and admin interface methods from the original file]
    ...
    public function ajax_debug_cron() {
        ...
        $cron_info['stuck_jobs'] = $stuck_jobs;
    ...
    public function ajax_cleanup_stuck_jobs() {
        ...
        // Schedule immediate processing
        $this->schedule_cron_event_improved(0);

        global $wpdb;
        $affected_rows = $wpdb->rows_affected;

        error_log('SFAIC: Reset ' . $affected_rows . ' stuck jobs');

        wp_send_json_success('Reset ' . $affected_rows . ' stuck jobs');
    }

    // [Include remaining admin interface methods - add_jobs_menu, enqueue_admin_scripts, render_jobs_page, etc.]
    // [These methods remain the same as in the original file]
    ...
    /**
     * Helper method to render table rows consistently
     */
